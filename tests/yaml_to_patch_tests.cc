...
PatchData:
  ...
  suggested_samplerate: 32000
  suggested_blocksize: 128
  bypassed_modules:
    - 1
    - 3
)";
	...
	CHECK(unsigned(pd.knob_sets[0].set[0].midi_chan) == 0);
	...
	CHECK(unsigned(pd.knob_sets[0].set[1].midi_chan) == 3);
	...
	CHECK(unsigned(pd.knob_sets[0].set[2].midi_chan) == 0);
	...
	CHECK(unsigned(pd.knob_sets[1].set[0].midi_chan) == 0);
	...
	CHECK(pd.module_states[0].state_data.size() == 25);
	CHECK(pd.module_states[0].state_data[0] == 'T');
	CHECK(pd.module_states[0].state_data[5] == 'L');
	CHECK(pd.module_states[0].state_data[10] == '\n');
	CHECK(pd.module_states[0].state_data[11] == 'A');
	...
	CHECK(pd.suggested_samplerate == 32000);
	CHECK(pd.suggested_blocksize == 128);

	CHECK(pd.bypassed_modules.size() == 2);
	CHECK(pd.is_module_bypassed(1));
	CHECK(pd.is_module_bypassed(3));
	CHECK(!pd.is_module_bypassed(0));
	CHECK(!pd.is_module_bypassed(2));
}
